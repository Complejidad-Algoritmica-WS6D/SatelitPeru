...
class ListRoutesVillages:
    ...
    def getRoutesVillagesBySourceId(self, sourceId):
        return [route_village for route_village in self.list if route_village.getSourceId() == sourceId]
    
    def getRoutesVillagesBySourceVillage(self, sourceVillage):
        return [route_village for route_village in self.list if route_village.getSourceVillage() == sourceVillage]
    
    def addListRoutesVillages(self, listRoutesVillages):
        self.list = listRoutesVillages
    
    def getRoutesVillagesBySourceDepartment(self, sourceDepartment):
        newList = ListRoutesVillages()
        list2 = []
        for route_village in self.list:
            if route_village.getSourceDepartament() == sourceDepartment:
                list2.append(route_village)
                
        newList.addListRoutesVillages(list2)
        print('nueva lista', (newList).__str__())
        return newList
